...
| `thinking-text-shimmer` | 文字扫光思考动效 | User request | AI thinking / generation status text | loading, thinking, text feedback | HTML, CSS text background clip | LM, DM | A: small and direct copy | `snippets/thinking-text-shimmer/index.html` |
...
| `particle-input-ambient` | 输入框后弥散点阵动效 | Downloads/Homepage 0311_副本/输入框和 title 效果调整.html | input/card background | ambient, feedback, premium feel | HTML, CSS keyframes, canvas particles, JS delayed pointer | LM, DM | B: reusable background layer; preview input is demo-only | `snippets/particle-input-ambient/index.html` |
